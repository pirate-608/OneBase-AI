...
-   **纯容器模式：** 执行`onebase build --with-ollama -g` 和 `onebase serve --with-ollama -g -d`，系统将自动拉起 Ollama 容器，并打通网络映射。**首次启动后需执行 `docker exec -it onebase_ollama ollama run <模型名>` 下载模型。**
...
-   **纯容器模式：** 执行`onebase build --with-xinference -g` 和 `onebase serve --with-xinferece -g -d`。Xinference 容器将在后台启动。你可以访问 `http://localhost:9997` 的 Web 界面一键下载管理 ModelScope 的模型。
...
-   **纯容器模式：** 执行`onebase build --with-vllm -g` 和 `onebase serve --with-vllm -g -d`。容器启动时，将自动读取 `onebase.yml` 中的大模型 ID 并挂载 vLLM 容器执行推理！


⚠️ **建议：** 永远不要在 Web 服务容器内直接加载模型权重（这会导致镜像暴涨至几十G，且极易导致显存溢出）。正确的做法是将模型“推理服务化”。我们强烈推荐使用 Xinference 或 vLLM 来接管你下载的本地模型，并将其转化为 OneBase 可用的 API 接口。